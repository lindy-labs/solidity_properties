...
contract ERC1155 is Context, IERC1155, ERC165, IERC1155MetadataURI {
    ...
    function balanceOf(address account, uint256 id) public view   returns (uint256 balance) {
        //require(account != address(0), "ERC1155: balance query for the zero address"); // 2x calls to this function fails with this require :-(
    ...
    function mintBatch(address to, uint256[2] memory ids, uint256[2] memory amounts, bytes memory data) public  {
     ...
     */
    /// @notice emits TransferBatch  
    function _mintBatch(address to, uint256[2] memory ids, uint256[2] memory amounts, bytes memory data) internal  {
        require(to != address(0), "ERC1155: mint to the zero address");
        require(ids.length == amounts.length, "ERC1155: ids and amounts length mismatch");

        address operator = _msgSender();

        //_beforeTokenTransfer(operator, address(0), to, _fromStaticToDynArray(ids), _fromStaticToDynArray(amounts), data);

        /*for (uint i = 0; i < ids.length; i++) {
            if(_balances[ids[i]][to] + amounts[i] >= _balances[ids[i]][to] && _balances[ids[i]][to] + amounts[i] >= amounts[i])
                _balances[ids[i]][to] += amounts[i];
        }*/
        if(_balances[ids[0]][to] + amounts[0] >= _balances[ids[0]][to] && _balances[ids[0]][to] + amounts[0] >= amounts[0])
                _balances[ids[0]][to] += amounts[0];
        if(_balances[ids[1]][to] + amounts[1] >= _balances[ids[1]][to] && _balances[ids[1]][to] + amounts[1] >= amounts[1])
                _balances[ids[1]][to] += amounts[1];
